...
//        dailyCondimed[each["date"].trim()] = int.parse(each["dailyconfirmed"]);
...
// check if d1-m1 is greater than d2-m2
// 0 - false 1- greater 2 - equal
int isGreater(int d1, int m1, int d2, int m2) {
  print(d1);
  print(m1);
  print(d2);
  print(m2);
...
Future getAllIndiaData() async {
  ...
  var response =
      await http.get(kAllIndiaDataUrl).timeout(Duration(seconds: 10));
  var response2 = await http.get(kStateDataUrl).timeout(Duration(seconds: 10));
  ...
  if (response.statusCode == 200) {
    try {
      ...
      try {
        ...
        int result = isGreater(d2, m2, d1, m1);
        if (result == 1) {
          data.add({
            "dailyconfirmed": totalData["statewise"][0]["deltaconfirmed"],
            "dailydeceased": totalData["statewise"][0]["deltadeaths"],
...
//          print("%");
          ...
          // we need to take the greatest one
          var lastData = data.removeLast();
          if (int.parse(lastData["totalconfirmed"]) >=
              int.parse(totalData["statewise"][0]["confirmed"])) {
            data.add(lastData);
          } else {
            data.add({
